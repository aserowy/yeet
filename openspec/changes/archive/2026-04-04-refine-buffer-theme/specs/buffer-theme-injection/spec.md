...
### Requirement: BufferTheme border_fg ANSI string field
**Reason**: The `border_fg` ANSI string field is unused — border rendering uses `border_fg_color`/`border_bg_color` (now `border_fg`/`border_bg`) with ratatui's `Block::border_style()`.
**Migration**: Remove references to the old `border_fg` String field. Use `border_fg: Color` instead.
