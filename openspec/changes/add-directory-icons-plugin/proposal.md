...
- Expand the `on_bufferline_mutate` hook to provide the **complete bufferline** together with **wrapping buffer metadata object** as context. The buffer metadata is exposed as a read-only `buffer` object (`ctx.buffer`) on the hook context table, containing `type` (string matching `BufferType` enum — e.g., `"directory"`, `"content"`, `"help"`, `"quickfix"`, `"tasks"`) and optionally `path` (parent path for directory buffers, file path for content buffers; absent/nil for buffer types without an associated path). Using a dedicated metadata object ensures the API is extensible — future metadata fields can be added to `ctx.buffer` without breaking existing plugin code or changing the mutable field surface. The hook fires for **all buffer types**, not just directory buffers.
- Introduce a `BufferType` Rust enum (with variants `Directory`, `Content`, `Help`, `Quickfix`, `Tasks`) used by `invoke_on_bufferline_mutate` callers instead of passing raw `&str` values. The enum maps to the corresponding lowercase string injected into the Lua hook context, making invocations type-safe and preventing typos or invalid buffer type strings at compile time.
...
- Plugin-specific documentation (e.g., `DirectoryIconsColor*` token reference, plugin usage) is maintained in each plugin's own `docs/help/` directory, not in the core `docs/help/` files. The core `:help` command is extended to discover and include help pages from plugin `docs/help/` directories at runtime, in addition to the built-in compile-time help pages. When a topic matches both a core help page and a plugin help page, the core page takes priority.
...
- `buffer`: `@yeet-buffer` shared prefix model expands with a non-editable icon-column segment for all buffer definitions, including default-width behavior (`0` by default; `1` set via plugin `on_window_create` hook). The `on_bufferline_mutate` hook now fires for all buffer types and exposes the full bufferline (prefix, content, search_char_position, signs, icon) plus a read-only `buffer` metadata object (`ctx.buffer`) with `type` and `path` fields. A `BufferType` Rust enum is introduced for type-safe buffer type passing in `invoke_on_bufferline_mutate` (mapped to lowercase strings in Lua). The `icon_style` field is removed; plugins style content by mutating the Ansi string directly. Directory names in bufferline content end with a trailing slash. All public-facing hook APIs use object-based metadata to ensure extensibility without breaking changes.
...
- `help`: The `:help` command is extended to discover help pages from plugin `docs/help/` directories at runtime, in addition to built-in compile-time help pages. Core help pages take priority when names overlap.
...
- Introduces `BufferType` Rust enum for type-safe buffer type passing in hook invocations, replacing raw `&str` values.
...
- Plugin-specific documentation moves from core `docs/help/` to plugin `docs/help/` directories. The `:help` command gains runtime discovery of plugin help pages.
- Plugin help pages are searchable via `:help` alongside core help; core pages always take priority on name overlap.
