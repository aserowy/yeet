...
- [ ] 1.3 Define new per-bufferline mutation hook interface: the hook receives the complete bufferline (mutable) and the given window with all metadata; the plugin directly mutates the bufferline in-place (sets icon, colors text)
...
- [ ] 1.8 Register plugin `on_window_create` hook to set shared `@yeet-buffer` icon-column width to `1`
