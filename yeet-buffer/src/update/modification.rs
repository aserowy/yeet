...
pub fn update(
    ...
    match modification {
        TextModification::DeleteLine => {
            ...
            cursor::set_to_inbound_position(cursor, buffer, mode);
        ...
        TextModification::DeleteMotion(delete_count, motion) => {
            ...
            for _ in 0..*count {
                cursor::update_by_direction(mode, cursor, buffer, delete_count, motion);
            }
            ...
            if is_line_delete(motion) {
                ...
                if let Some(cng) = update(mode, cursor, buffer, &count, action) {
                    changes.extend(cng);
                }
            } else {
        ...
        TextModification::Insert(raw) => {
            ...
            if let Some((line, is_new_line)) = line {
...
fn get_line_or_create_on_empty<'a>(
    ...
    if buffer.lines.is_empty() {
        ...
        Some((&mut buffer.lines[0], true))
    } else {
        let line = buffer.lines.get_mut(cursor.vertical_index)?;
